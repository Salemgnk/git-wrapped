...
const Q_MAIN = `query($user:String!,$from:DateTime!,$to:DateTime!){
  user(login:$user){ id avatarUrl createdAt
...
// contributionsCollection refuse une plage > 1 an ("The total time spanned by 'from'
// and 'to' must not exceed 1 year"). On découpe donc en fenêtres d'au plus un an,
// de la plus récente à la plus ancienne, contiguës et couvrant toute la période.
export function yearWindows(fromD, toD) {
  const out = [];
  let end = toD;
  while (end >= fromD) {
    const e = new Date(end + "T00:00:00Z");
    const s = new Date(e); s.setUTCFullYear(e.getUTCFullYear() - 1);
    const start = isoDay(s.getTime() + DAY) < fromD ? fromD : isoDay(s.getTime() + DAY);
    out.push({ from: start, to: end });
    if (start <= fromD) break;
    end = isoDay(Date.parse(start) - DAY);
  }
  return out;
}

export async function fetchWrapped(user, period, token, fetchImpl = fetch) {
  const { fromD, toD, to } = bounds(period);
  // Une requête par fenêtre d'un an ; on s'arrête dès qu'on passe sous la création
  // du compte (le préréglage « tout » part de 2008, la plupart des années sont vides).
  const byRepo = new Map();
  let u = null, createdD = null;
  for (const w of yearWindows(fromD, toD)) {
    if (createdD && w.to < createdD) break;
    const data = await graphql(Q_MAIN,
      { user, from: w.from + "T00:00:00Z", to: w.to + "T23:59:59Z" }, token, fetchImpl);
    if (!data.user) { const e = new Error("user not found"); e.code = 404; throw e; }
    if (!u) { u = data.user; createdD = (data.user.createdAt || "").slice(0, 10) || null; }
    ...
    for (const x of data.user.contributionsCollection.commitContributionsByRepository) {
      if (x.repository.isPrivate) continue;
      const key = x.repository.owner.login + "/" + x.repository.name;
      const seen = byRepo.get(key);
      if (seen) seen.count += x.contributions.totalCount;
      else byRepo.set(key, { name: x.repository.name, owner: x.repository.owner.login,
        count: x.contributions.totalCount });
    }
  }
  // Période effective : inutile de remonter avant l'ouverture du compte.
  const startD = createdD && createdD > fromD ? createdD : fromD;
  const from = startD + "T00:00:00Z";
  const reposByCommits = [...byRepo.values()].sort((a, b) => b.count - a.count);
  ...
  return { user, from: startD, to: toD, id: u.id, avatar: u.avatarUrl, reposByCommits, commits, languages };
}
