export const REPO_COMMIT_CAP = 50;
...
export function devScore(commits) {
  ...
  let commitsPrime = 0;
  for (const n of byRepo.values()) commitsPrime += Math.min(n, REPO_COMMIT_CAP);
  const detail = common(commits, DEV_LINES_CAP);
  return { score: commitsPrime + detail.joursActifs * ACTIVE_DAY_WEIGHT + Math.floor(detail.lignes / 100), detail };
}
